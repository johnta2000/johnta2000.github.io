...
test('independent comment popups, replies, failure recovery, and responsive positioning', async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => {
      const [a, b] = [...document.querySelectorAll('.comment-thread-panel')].map(node => node.getBoundingClientRect());
      return b.top >= a.bottom || a.top >= b.bottom;
    });
    const boxes = await cards.evaluateAll(nodes => nodes.map(node => { const r = node.getBoundingClientRect(); return { top: r.top, bottom: r.bottom, left: r.left, right: r.right }; }));
    assert.ok(boxes.every(box => box.left >= 0 && box.right <= 1600));
    assert.ok(boxes[1].top >= boxes[0].bottom || boxes[0].top >= boxes[1].bottom);
