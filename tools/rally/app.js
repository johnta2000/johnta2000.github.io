...
function openDialog(title,description,formHtml,onSubmit){ el.dialogRoot.innerHTML=`<div class="dialog-backdrop"><section class="dialog"><button class="dialog-close">×</button><span class="eyebrow">Rally project room</span><h2>${title}</h2><p>${description}</p><form>${formHtml}<button class="primary" type="submit">Save</button></form></section></div>`; const backdrop=el.dialogRoot.firstElementChild, form=backdrop.querySelector("form"); backdrop.querySelector(".dialog-close").onclick=closeDialog; backdrop.onmousedown=(event)=>{if(event.target===backdrop)closeDialog()}; form.onsubmit=async(event)=>{event.preventDefault();const button=form.querySelector("[type=submit]");button.disabled=true;try{await onSubmit(Object.fromEntries(new FormData(form)))}catch(error){showToast(error.message||"Could not save");button.disabled=false}}; }
...
function openInvite(member){ const inviteable=data.members.filter((m)=>!["admin","leader"].includes(m.role)&&(m.status!=="confirmed"||!m.email)); if(!inviteable.length)return showToast("Everyone has claimed their profile"); const selected=member||inviteable[0]; openDialog(`Invite ${escapeHtml(selected.name)}`,"Connect their email to the profile already carrying their room, task, and travel assignments.",selectField("Crew profile","memberId",inviteable.map((m)=>[m.id,m.name]),selected.id)+field("Email address","email",selected.email,"email"),async(values)=>{await act("invite-member",values,`Invite prepared for ${selected.name}`,false);closeDialog();const subject=encodeURIComponent(`Join ${data.name} on Rally`),body=encodeURIComponent(`You’re invited to our ${data.name} project room. Sign in with ${values.email}:\n\n${location.origin}${href("crew")}`);location.href=`mailto:${values.email}?subject=${subject}&body=${body}`;}); }
...
async function convexQuery(path,args){return convexCall("query",path,args)} async function convexMutation(path,args){return convexCall("mutation",path,args)}
