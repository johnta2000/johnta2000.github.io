...
function renderStay(){ const map=memberMap(); el.page.innerHTML=heading("Lodging plan","Stay","Hotel rooms, confirmations, capacity, and who sleeps where.",`<button id="addRoom" class="primary">＋ Add room</button>`)+`<div class="room-grid">${data.rooms.map((room,index)=>`<article class="room-card"><header><span>Room ${index+1}</span><button data-room-edit="${room.id}">•••</button></header><h3>${escapeHtml(room.roomType)}</h3><p>${escapeHtml(room.hotel)}${room.confirmation?` · Confirmation ${escapeHtml(room.confirmation)}`:""}</p>${room.notes?`<p class="room-note">${escapeHtml(room.notes)}</p>`:""}${room.totalCost?`<strong class="room-price">${escapeHtml(room.totalCost)}</strong>`:""}<div class="meter"><i style="width:${Math.min(100,room.memberIds.length/room.capacity*100)}%"></i></div><small>${room.memberIds.length} assigned · ${room.capacity-room.memberIds.length} open</small><div class="row-list">${room.memberIds.map((id)=>`<div class="row"><strong>${escapeHtml(map[id]?.name||"Crew")}</strong><span>${escapeHtml(map[id]?.origin||"")}</span></div>`).join("")}<button class="secondary" data-assign-room="${room.id}">＋ Assign person</button></div></article>`).join("")||`<div class="empty">No rooms added yet.</div>`}</div>`; document.getElementById("addRoom").onclick=()=>openRoom(); document.querySelectorAll("[data-room-edit]").forEach((b)=>b.onclick=()=>openRoom(data.rooms.find((r)=>r.id===b.dataset.roomEdit))); document.querySelectorAll("[data-assign-room]").forEach((b)=>b.onclick=()=>openAssign(b.dataset.assignRoom)); }

function renderTravel(){ const map=memberMap(); el.page.innerHTML=heading("Arrival board","Travel","Flight legs and ground transportation in one shared place.",`<div><button id="addCar" class="secondary">＋ Rental car</button> <button id="addFlight" class="primary">＋ Link flight</button></div>`)+`<div class="flight-board">${data.travel.map((trip)=>`<article class="flight-card editable-card"><button class="card-menu" data-flight-edit="${trip.id}" aria-label="Edit flight">•••</button><div><small>${escapeHtml(trip.departure)}</small><strong>${escapeHtml(trip.origin)}</strong></div><span>✈</span><div><small>${escapeHtml(trip.arrival)}</small><strong>${escapeHtml(trip.destination)}</strong></div><small>${escapeHtml(map[trip.memberId]?.name||"Crew")} · ${escapeHtml(trip.airline)} ${escapeHtml(trip.number)}${trip.confirmation?` · ${escapeHtml(trip.confirmation)}`:""}</small></article>`).join("")||`<div class="empty">No flights linked yet. Add each layover as its own leg.</div>`}</div><section class="section-card"><header><div><span class="eyebrow">Ground transportation</span><h2>Rental cars</h2></div></header><div class="car-grid">${data.cars.map((car)=>`<article class="car-card editable-card"><button class="card-menu" data-car-edit="${car.id}" aria-label="Edit rental car">•••</button><h3>${escapeHtml(car.company)} · ${escapeHtml(car.vehicle)}</h3><p>${escapeHtml(car.pickup)} → ${escapeHtml(car.dropoff)}</p><small>Driver: ${escapeHtml(map[car.driverId]?.name||"Unassigned")}</small></article>`).join("")||`<div class="empty">No rental car stored.</div>`}</div></section>`; document.getElementById("addFlight").onclick=()=>openFlight(); document.getElementById("addCar").onclick=()=>openCar(); document.querySelectorAll("[data-flight-edit]").forEach((button)=>button.onclick=()=>openFlight(data.travel.find((trip)=>trip.id===button.dataset.flightEdit))); document.querySelectorAll("[data-car-edit]").forEach((button)=>button.onclick=()=>openCar(data.cars.find((car)=>car.id===button.dataset.carEdit))); }
...
function openFlight(trip){openDialog(trip?"Edit flight leg":"Link a flight leg","For layovers, keep each flight as a separate leg.",selectField("Traveler","memberId",data.members.map((m)=>[m.id,m.name]),trip?.memberId||"")+`<div class="form-row">${field("Airline","airline",trip?.airline||"")}${field("Flight number","number",trip?.number||"")}</div>`+field("Confirmation","confirmation",trip?.confirmation||"","text",false)+`<div class="form-row">${field("From","origin",trip?.origin||"")}${field("To","destination",trip?.destination||"")}</div><div class="form-row">${field("Departs","departure",trip?.departure||"","datetime-local")}${field("Arrives","arrival",trip?.arrival||"","datetime-local")}</div>`,async(values)=>{await act("save-flight",{...values,id:trip?.id||""},trip?"Flight updated":"Flight leg added");closeDialog()},trip?`<button id="deleteFlight" class="danger-button" type="button">Delete flight</button>`:"");if(trip)wireDangerButton("deleteFlight",async()=>{await act("delete-flight",{id:trip.id},"Flight deleted");closeDialog()});}
